...
def Submit_Purchase(data:Schema.SubmitRentProperty, username:str, db:Session):
    if username == username:
        try:
            new_record = model.RentRecord(property=data.number, owner=data.owner, customer=username, rent = data.rent, downpayment=data.downpayment, tenure=data.tenure, bookingdate=datetime.today().strftime('%Y-%m-%d'), verification = False)
            db.add(new_record)
            db.commit()
