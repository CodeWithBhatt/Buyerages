...
def All_Property(request:Request, current_user:Schema.UserData = Depends(Auth.get_current_user), db : Session = Depends(db)):
    if current_user.user == 'admin':
        ...
        result = {"BuyProperty":property[0], "RentProperty":property[1]}
        # return {"data":result} # remove response_class from decorator
        return templates.TemplateResponse("property.html", {"request":Request, "data":result})
    else:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Only Admin can access this")

@router.put('/property', status_code=status.HTTP_202_ACCEPTED)
def verify_record(number:str, current_user:Schema.UserData = Depends(Auth.get_current_user), db : Session = Depends(db)):
    if current_user.user == 'admin':
...
@router.get('/records', response_class=HTMLResponse, status_code=status.HTTP_200_OK)
...
@router.put('/records', status_code=status.HTTP_202_ACCEPTED)
