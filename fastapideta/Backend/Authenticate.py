...
def Find_User(data:Schema.Add_Account, db:Session):
    user = db.query(model.User).filter(or_(model.User.usn == data.usn, model.User.email == data.email)).first()
    if not user:
        return user
    else:
        return True

def Add_User(data:Schema.Add_Account, db:Session):
    try :
        new_user = model.User(name=data.name, usn=data.usn.lower(), email=data.email, sex=data.sex, phone=data.phone, password=hash_pass(data.password), role='user')
        db.add(new_user)
        db.commit()
        db.refresh(new_user)
        return new_user
...
def get_user(username:str, db:Session):
    if len(username) == 10:
        user = db.query(model.User).filter(model.User.usn == username).first()
    else:
